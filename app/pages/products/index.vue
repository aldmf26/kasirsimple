...
onMounted(async () => {
  if (!store.value) {
    console.warn("⚠️ Store is null, attempting to fetch...");
...
<template>
  <div class="h-full flex flex-col bg-gray-50">
    <!-- Header -->
    <div
      class="px-8 py-6 border-b border-gray-200 bg-white flex justify-between items-center shrink-0 shadow-sm"
    >
      <div>
      ...
      >
        ...
        >
          ...
          <template #stock-data="{ row }">
            <UBadge
              :color="row.stock <= (row.min_stock || 5) ? 'error' : 'success'"
              variant="subtle"
              >{{ row.stock }}</UBadge
            >
          </template>
